...
# Train E3/E4 event-drop variants concurrently on two V100 GPUs.
...
GPU assignment:
  ...
  GPU 1: E4, joint z/h distillation with temporal event dropout and masked z loss

Both runs use dataset=dsec_benchmark_clean and start from the same pretrained
encoder initialization as E1/E2; they do not fine-tune the old checkpoints.
EOF
}
...
if [ -z "$OUTPUT_ROOT" ]; then
  OUTPUT_ROOT="$PROJECT_ROOT/outputs/v100_event_dropout_$(date +%Y%m%d_%H%M%S)"
fi
...
launch() {
  ...
  printf '[event-drop-training] GPU %s: starting %s\n' "$gpu" "$name"
...
for PID in "${PIDS[@]}"; do
  if wait "$PID"; then
    printf '[event-drop-training] complete: %s\n' "${NAMES[$INDEX]}"
  else
    printf '[event-drop-training] failed: %s (see %s/logs/%s.log)\n' \
...
trap - INT TERM
printf '[event-drop-training] outputs: %s\n' "$OUTPUT_ROOT"
exit "$STATUS"
